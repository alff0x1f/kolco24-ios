...
- Заявить сбор **геолокации (Precise Location)**: назначение — запись GPS-трека команды
  и анти-фрод при взятии КП (координата фиксируется в момент отметки).
- Data Use: App Functionality. **Linked to the user** — да (данные связаны с командой/участником).
  Tracking — **нет** (`NSPrivacyTracking = false`, доменов трекинга нет).
- Остальные типы данных не собираются.
