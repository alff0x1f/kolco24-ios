...
- [x] фон: линейный градиент `#171D25 → #232A33` (≈155°), фиксированные `Color(hex:)`
- [x] inset-тени «утопленности» + диагональная штриховка white α≈0.025 (мотив `Canvas`)
- [x] глиф бесконтактной оплаты — три дуги цветом `#E6EAF0`
- [x] номер — белый mono (28, bold) с лёгкой тенью
- [x] убрать красные полоски (атрибут светлой плитки)
- [x] убедиться, что `PhotoTileView`/`MiniCPBadge` не затронуты
- [x] собрать проект — сборка проходит (`** BUILD SUCCEEDED **`)
