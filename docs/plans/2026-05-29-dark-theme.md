...
- Только перекраска через токены. Никаких новых экранов, переключателей, persistence.
- Контент, одинаковый в обеих темах, **не трогаем:** градиенты-превью фото КП, белый
  светоотражающий `CPBadge`, белый текст и штриховка на тёмном «герое».
- **Вне объёма:** полная визуальная замена `NFCTileView` на «чип-карту» из `design_dark.html`
  (контактные дуги, тёмный градиент). В этом плане NFC-плитка перекрашивается только токенами.
...
**Особый случай `NFCTileView` (`MarksView.swift:94`):** заменить canvas-fill `.white` на `Color.card`,
номер уже `Color.ink` (адаптивен), полоски `brandRed` (адаптивен). Полную замену глифа — не делать.
...
- Проверить, что `grep -rnE "\.white|Color\.black|\.black\.opacity" kolco24/*.swift` не содержит
  оставшихся «поверхностных» литералов (только герой/контент).

## Файлы, которые меняются

- `kolco24/DesignTokens.swift` — хелпер + адаптивные токены (Шаг 1)
- `kolco24/SharedComponents.swift`, `MarksView.swift`, `LegendView.swift`, `ScanSheet.swift`,
