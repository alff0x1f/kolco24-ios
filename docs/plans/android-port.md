...
- Отложено на этап 9: «Очистить трек» (guard «не во время записи»), LAN-lease.
- Профили Precise/Economy + тумблер «Экономия батареи» — **не портируются** (сняты с этапа 9): на iOS `liveUpdates(.fitness)` сам управляет питанием (нет настраиваемого интервала, авто-пауза на стоянках), «усыпить» геолокацию нельзя.

### Этап 9. LAN-режим и настройки

✅ выполнен — см. [детальный план](completed/20260711-android-port-stage9.md).

- Полный local-mode: `/sync/`-манифест, lease на 12 ч (`RaceLease`/`LeaseHolder`/`SyncCoordinator`-actor), переключение источников cloud/LAN (реальный пин-гард репозиториев), экран настроек (тема, LAN-тумблер, очистка трека, скрытая отладка, версия).
- «Инфо о чипе» (GET_VERSION) перенесено в этап 10 (вместе с рядом «Администратор»).

### Этап 10. Админ-режим

✅ выполнен — см. [детальный план](completed/20260711-android-port-stage10.md).

- Логин (bearer в Keychain, один JSON-item), судейский скан старт/финиш (write-once строки + дуал-таргет дрейн), проверка КП-чипа и браслета (read-only, полностью оффлайн), провижининг (двухтаповый флоу со сверкой uid + header-last запись/read-back).
- Bearer добавлен к обоим клиентам поверх подписи (в canonical не входит); NFC-сканер обобщён per-tag обработчиком (session-менеджмент этапа 5 не тронут).
- «Инфо о чипе» (GET_VERSION-проба чипа) — **не портируется** (решение при планировании; `chipModelFromVersion` остаётся в Core как мёртвый протестированный код этапа 1).
- Провижининг — **самая рискованная фича**: запись чипов проверяется руками на реальных NTAG213/215/216 (Post-Completion; NFC в симуляторе не работает).
