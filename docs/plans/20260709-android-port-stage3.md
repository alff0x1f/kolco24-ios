...
- [x] полный `xcodebuild test` зелёный (все кейсы этапов 0–2 + новые): `** TEST SUCCEEDED **`, 470 тест-кейсов passed, 0 failed, 2 skipped (`LiveServerSmokeTests` env-gated + 1 UI). iPhone 16 sim `F5E33361-5D27-4A82-8347-39005F2A246E`.
- [x] сверка покрытия: `ApiClientTests` 54 `@Test` (48 зеркал + retry-матрица `SigningTest` + 1 бонус; header «Зеркало …», retry-блок «403-retry матрица (Зеркало `SigningTest.kt` `interceptor_*`)»), `ServerTimeSamplerTests` 8 (6 из 9 кейсов принадлежат чистому сэмплеру + бонус; header «Зеркало …»), `RaceRepositoryTests` 12/12, `TeamRepositoryTests` 18/18, `LegendRepositoryTests` 30 (29 зеркал + 1 бонус), `MemberTagsRepositoryTests` 32/32 — все с header «Зеркало …», бонусы помечены `// MARK: - БОНУС-тесты`. `DtoDecodingTests` 16 — корректно БОНУС-сьют целиком (у wire-DTO нет Android-юнит-зеркала, kotlinx.serialization доверенная; задокументировано в шапке), потому без «Зеркало …» header.
- [x] grep-инварианты: `import UIKit|SwiftUI` отсутствует в `Core/`/`Model/`/`Data/`/`Net/` (пусто). `import GRDB` (line-anchored `^import GRDB`) вне `Data/` — только `kolco24Tests/GRDBSmokeTests.swift` (пред-существующий стейдж-2 smoke-тест GRDB, есть на `main`, не изменён этапом 3). `Net/` без реального `import GRDB` — `URLSessionTransport.swift` импортирует только `Foundation`; substring-совпадение `grep -l "import GRDB"` там ложное (строка «`Net/` без `import GRDB`» в prose-комментарии). Инвариант держится.
- [x] UI не тронут: `git diff --name-only main...HEAD` не содержит ни одного из 8 корневых UI-файлов и `project.pbxproj` (только `Core/`, `Net/`, `Data/Repositories/`, тесты, план).
- [x] smoke против живого сервера — сьют `LiveServerSmokeTests` env-gated (`LIVE_API_SMOKE`), скипнут в этом прогоне. Живой 200 в этом окружении не подтверждён: `xcodebuild` НЕ пробрасывает `LIVE_API_SMOKE` в hosted-тест-процесс симулятора (см. вывод задачи 8) — сьют скипается даже с переменной в родительском шелле. Гейт корректен, локальный сьют зелёный; живой прогон возможен только через переменную в схеме/тест-плане. Отмечено [x] по договорённости задачи 8 (best-effort, не блокер).
- [x] инварианты потока перечитаны по чек-листу (все 4 репозитория + `ApiClient`): **данные-раньше-ETag** — Race/Team/Legend/MemberTags все делают `replaceAll…` до `upsert(SyncMeta…)`, ETag в отдельной транзакции; **кросс-origin-инвалидация** — `deleteEtag(otherOrigin)` ДО замены (Race:78, Team:98, Legend:128, MemberTags:113–116, +synced-маркер); **повторный pin-guard после 200** — Team:93–94, Legend:123–124, MemberTags:107–108 (Race без guard'а — гонки глобальны, by design); **POST-не-ретраится** — `ApiClient.send` retry-гейт требует `method == "GET"||"HEAD"` (ApiClient.swift:242–244), POST не проходит; **LAN без `onServerTime`** — `ApiClients.makeLocal` передаёт `onServerTime: nil` (URLSessionTransport.swift:112), `transportAndSample` сэмплит только `if let onServerTime`.
