...
- [ ] создать `Config/App.xcconfig` (в git) с non-optional `#include "Secrets.xcconfig"` и комментарием про гейт
- [ ] создать `Config/Secrets.example.xcconfig` с плейсхолдерами, `$()`-трюком и комментарием «скопируй в Secrets.xcconfig и заполни»
- [ ] создать `Config/Secrets.xcconfig` с реальными значениями (боевые значения взять из `kolco24_app_v2/local.properties`)
- [ ] добавить `Config/Secrets.xcconfig` в `.gitignore`
- [ ] проверить: `git status` не показывает `Secrets.xcconfig`, показывает `App.xcconfig` и example
- [ ] тесты: нет Swift-кода в этой задаче — проверка цепочки целиком в `SecretsTests` (Task 3)
...
- [ ] создать `kolco24/Info.plist` с 4 кастомными ключами `Kolco24*` = `$(VAR)` и `NSAppTransportSecurity` → `NSAllowsLocalNetworking = YES`
- [ ] в pbxproj: добавить file reference на `Config/App.xcconfig`, `Config/Secrets.example.xcconfig` и `kolco24/Info.plist`
- [ ] в pbxproj: выставить `baseConfigurationReference` на `App.xcconfig` для Debug/Release конфигураций app-таргета (не тестовых)
- [ ] в pbxproj: `INFOPLIST_FILE = kolco24/Info.plist` для app-таргета, `GENERATE_INFOPLIST_FILE` оставить `YES`
- [ ] собрать: `xcodebuild -project kolco24.xcodeproj -scheme kolco24 -destination 'platform=iOS Simulator,name=iPhone 16' build` — успех
- [ ] проверить продукт сборки: `plutil -p <DerivedData>/.../kolco24.app/Info.plist` — значения подставлены (не `$(...)`), `NSAllowsLocalNetworking` присутствует, генерируемые ключи на месте (`UIApplicationSceneManifest` и особенно `NFCReaderUsageDescription` — он нужен приложению уже сейчас)
- [ ] тесты: нет Swift-кода — проверка в `SecretsTests` (Task 3)
...
- [ ] создать `enum Secrets` со свойствами `apiBaseURL`, `appKeyId`, `appSecret`, `localAPIBaseURL`, читающими `Bundle.main`; отсутствие/пустота → `fatalError` с именем ключа и подсказкой про `Config/Secrets.xcconfig`
- [ ] написать `SecretsTests` (Swift Testing): все 4 значения непустые и не содержат `$(`
- [ ] написать тесты URL-ов: `URL(string:)` не nil; схема `https` у `apiBaseURL`, `http` у `localAPIBaseURL`; **`url.host` непустой у обоих** — именно host ловит сломанный `$()`-трюк (обрезанное `https:` парсится как валидный URL со схемой, но без host)
- [ ] прогнать тесты: `xcodebuild test ... -only-testing:kolco24Tests` — зелёные
...
- [ ] добавить SPM-пакет `https://github.com/groue/GRDB.swift`, up-to-next-major от 7.0.0, слинковать с app-таргетом (только с ним)
- [ ] написать смоук-тест: `import GRDB`, открыть in-memory `DatabaseQueue`, выполнить тривиальный запрос
- [ ] ⚠️ фолбэк: если `import GRDB` в тестовом таргете не компилируется (видимость через host app — известно хрупкое место SPM), добавить продукт GRDB также в зависимости `kolco24Tests`
- [ ] прогнать тесты — зелёные

### Task 5: Verify acceptance criteria

- [ ] `xcodebuild build` проходит с заполненным `Secrets.xcconfig`
- [ ] негативная проверка: временно переименовать `Config/Secrets.xcconfig` → сборка падает с ошибкой «could not find included file 'Secrets.xcconfig'» (missing `baseConfigurationReference` дал бы лишь warning — потому гейт через `#include` в `App.xcconfig`); вернуть файл
- [ ] `xcodebuild test` — весь набор зелёный
- [ ] `plutil -p` по продукту сборки: значения подставлены, `NSAllowsLocalNetworking = 1`
- [ ] приложение запускается в симуляторе (UI не изменился)
...
- [ ] в `android-port.md` пометить этап 0 ссылкой на этот план
- [ ] в `CLAUDE.md` добавить раздел про сборку с нуля: скопировать `Config/Secrets.example.xcconfig` → `Config/Secrets.xcconfig`, заполнить; упомянуть `Secrets.swift`, ATS и GRDB
