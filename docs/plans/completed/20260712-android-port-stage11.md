...
- **Единственный потребитель `statusUpdates` — `AppModel`.** `AsyncStream` допускает одного итератора; `AppModel.start()` запускает долгоживущий `Task` (`for await statusUpdates` — начальное значение приходит первой итерацией засеянного `.bufferingNewest(1)`-стрима, отдельный `await trustedClock.status` не нужен) и публикует `@Observable var clockStatus`. Все поверхности читают свойство, стрим больше никто не трогает.
...
- [x] `var clockStatus: ClockStatus = .noSync` (@Observable, публичное чтение); в `start()` — долгоживущий `Task`: `for await s in trustedClock.statusUpdates { clockStatus = s }` (идиома существующих подписок; отмена вместе с моделью). Отдельное `await trustedClock.status` перед циклом не нужно — `statusUpdates` это `.bufferingNewest(1)`-стрим, засеянный текущим статусом в `TrustedClock.init`, поэтому начальное значение приходит первой же итерацией
...
- [x] `PrivacyInfo.xcprivacy` (`NSPrivacyAccessedAPITypes`): `NSPrivacyAccessedAPICategoryUserDefaults` → `CA92.1` (свои `InstallId`/`ClockAnchorStore`/`ThemePreference`/`RaceLeaseStore`); `NSPrivacyAccessedAPICategorySystemBootTime` → `35F9.1` (`mach_continuous_time` в `SystemClockProviders`, `CoreLocationProvider`, `CLLocationMapping`, `CoreLocationTrackEngine`, `ApiClient` — везде измерение прошедшего времени; одна запись покрывает все). Плюс `NSPrivacyTracking = false`, пустой `NSPrivacyTrackingDomains`. `NSPrivacyCollectedDataTypes` — пять записей (все `Linked = true`, `Tracking = false`, назначение `AppFunctionality`; всё уходит с устройства привязанным к команде/организатору, Apple считает это «collected»; совпадает с App-Privacy-опросником в `docs/release.md §5`): `NSPrivacyCollectedDataTypePreciseLocation` (GPS-трек + координата взятия), `NSPrivacyCollectedDataTypePhotosorVideos` (JPEG-кадры фото-отметки через `ApiClient.uploadMarkPhoto` / frame drain), `NSPrivacyCollectedDataTypeEmailAddress` (Contact Info — email организатора при входе в админ-режим, уходит в теле `POST /app/login/` и хранится с токеном в Keychain), `NSPrivacyCollectedDataTypeDeviceID` (`install_id` — UUID установки в `X-Install-Id` и `source_install_id`), `NSPrivacyCollectedDataTypeUserID` (назначенные идентификаторы участников: номер команды, номера участников, NFC-UID браслетов). GRDB свой манифест везёт. Файл попал в бандл как ресурс — synchronized group это делает сам (в отличие от `Info.plist` исключение НЕ нужно; проверено — `membershipExceptions` содержит только `Info.plist`, файл доехал до `Release-iphonesimulator/kolco24.app/PrivacyInfo.xcprivacy`)
