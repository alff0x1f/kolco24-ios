...
**Manual verification:**
- когда появится сетевой код (этап 3): подписанный GET `/app/races/` до облака (HTTPS) и до LAN-сервера (cleartext, проверка `NSAllowsLocalNetworking` на реальном устройстве в одной сети с сервером). `NSLocalNetworkUsageDescription` (обязателен для доступа к локальной сети, TN3179) уже добавлен в Info.plist по итогам внешнего ревью — при первом LAN-трафике проверить системный промпт с этой строкой.
